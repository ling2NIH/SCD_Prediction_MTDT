...
# of the bar...
def waterfall_v2(shap_values, max_display=10, show=True, xlim=None,order=None):
    ...
    if num_features < len(values):
        yticklabels[0] = "other baseline variables" 
    ...
    ax3.set_xticklabels(["Predicted 5-year mortality (%)", "$ = "+format_value(fx, "%0.03f")+"$"], fontsize=12, ha="left")
    tick_labels = ax3.xaxis.get_majorticklabels()
    tick_labels[0].set_transform(tick_labels[0].get_transform(
    ) + matplotlib.transforms.ScaledTranslation(-168/72., 0, fig.dpi_scale_trans))
    ...
    tick_labels[0].set_color("#FF0000")
    tick_labels[1].set_color("#FF0000")
    ...
    tick_labels[0].set_transform(tick_labels[0].get_transform(
    ) + matplotlib.transforms.ScaledTranslation(-211/72., 0, fig.dpi_scale_trans))
    tick_labels[1].set_transform(tick_labels[1].get_transform(
...
def waterfall_legacy_v2(expected_value, shap_values=None, features=None, feature_names=None, max_display=10, xlim=None,order=None,show=True):
    """ Plots an explanation of a single prediction as a waterfall plot.
    ...
    # set the xlim to the user specified value
    if xlim is not None:
        ax.set_xlim(xlim)
    if show:
        plt.show()
