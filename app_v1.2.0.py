...
def import_dataset_SDC_sim_outcome(norm_mode='standard'):
    df_with_miss = pd.read_pickle('/scratch/ling2/FSL-Mate/PaddleFSL/examples/molecular_property_prediction/SCD/df_baseline_outcomes_io_na_nomiss30_v4.pkl')
...
learning_rate_ae = 0.014042537758462046

eta= 3.064097912304327e-06

alpha= 98.47058016064118

beta= 0.09862419473037817

gamma= 7.98995024019284e-05

delta = [1.867845689006929e-02, 16.14873363374915, 0.24598507609918738, 0.08011228875744171, 0.001721904454997685, 0.018416867725144413, 31.906937643040543, 0.03749735452214649, 0.0043699241092766405, 2.1074472710908885, 0.24860461313743257, 0.0725654224026344]
...
learning_rate= 0.016217536975895074
...
model.set_state_dict(paddle.load("./saved_models/model_multitask_deephit_v2.14_0.9829.pdparams"))
model.eval()

model_copy = ModelDeepHit_Multitask(input_dims, network_settings, outcome_configs  , autoencoder, log_writer)
model_copy.set_state_dict(paddle.load("./saved_models/model_multitask_deephit_v2.14_0.9829.pdparams"))
model_copy.eval()

x_mean = np.load("./data_info/x_mean.npy")
x_std = np.load("./data_info/x_std.npy")
feature_name = np.load("./data_info/feature_name.npy", allow_pickle=True)
X_example = pd.read_csv("./data_examples/example_scd_data.csv")
calibration_example = pd.read_csv("./data_examples/example_scd_for_calibration.csv")
batch_basis_eval = np.load("./data_info/batch_basis_eval.npy")
mean_list = np.load("./data_info/mean_long.npy")
std_list = np.load("./data_info/std_long.npy")
long_names = np.load("./data_info/long_names.npy", allow_pickle=True)
...
def conformal_mortality_prediction(
    ...
    """
    Run conformal prediction for mortality over a range of horizons.

    Parameters:
    - model: trained Paddle model with paddle_manual_conformal_interval interface
    - X_and_mask: full covariate+mask tensor (n_samples x dim)
    - E_train: array of event indicators (1 if event occurred)
    - T_train: array of event or censoring times (same length as E_train)
    - index: index of test patient (int)
    - max_horizon: number of time steps (e.g., 10 years)
    - alpha: conformal level

    Returns:
    - Dictionary with keys:
        'years', 'predicted_mortality', 'lower_bounds', 'upper_bounds',
        'corrected_lower_bounds', 'corrected_upper_bounds'
    """
...
with open("./saved_explainers/explainer_v2.14.dill","rb") as f:
